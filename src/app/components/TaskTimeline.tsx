  ...
  return (
    <div className="bg-[#0F172A] border border-[rgba(124,58,237,0.1)]">
      <div className="flex items-center justify-between p-4 border-b border-[rgba(124,58,237,0.1)]">
        <button onClick={prevQuarter} className="p-2 hover:bg-[rgba(124,58,237,0.05)] rounded">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={goToToday}
            className="px-3 py-1 text-sm bg-[rgba(124,58,237,0.1)] text-[#7C3AED] border border-[rgba(124,58,237,0.2)] hover:bg-[rgba(124,58,237,0.2)]"
          >
            Today
          </button>
        </div>
        <div className="flex items-center gap-2">
          ...
          <select
            ...
            className="px-2 py-1 bg-white border border-[rgba(124,58,237,0.1)] text-foreground text-xs"
          >
          ...
          <button onClick={nextQuarter} className="p-2 hover:bg-[rgba(124,58,237,0.05)] rounded">
      ...
      <div className="flex">
        <div className="w-56 flex-shrink-0 border-r border-[rgba(124,58,237,0.1)]">
          <div className="h-10 border-b border-[rgba(124,58,237,0.1)]" />
          {months.map(m => (
            <div key={m.toISOString()} className="h-10 border-b border-[rgba(124,58,237,0.05)] flex items-center px-3">
              <span className="text-xs font-medium text-muted-foreground">{format(m, 'MMMM')}</span>
            </div>
          ))}
        </div>
        ...
        <div className="flex-1 overflow-x-auto">
          <div className="min-w-[600px]">
            <div className="flex h-10 border-b border-[rgba(124,58,237,0.1)] relative">
              {months.map(m => {
                ...
                return (
                  <div
                    key={m.toISOString()}
                    className="absolute top-0 bottom-0 border-r border-[rgba(124,58,237,0.1)]"
                    style={{ left, width }}
                  >
                    <span className="text-[10px] text-muted-foreground px-1">{format(m, 'MMM')}</span>
                  </div>
                );
              })}
            </div>
            ...
            <div className="divide-y divide-[rgba(124,58,237,0.05)]">
              {visibleGroups.length === 0 && (
                <div className="p-12 text-center">
                  ...
                  <p className="text-muted-foreground">No tasks in this range</p>
                </div>
              )}
              {visibleGroups.map(group => (
                <div key={group.key}>
                  <div className="flex items-center gap-2 px-4 py-2 border-b border-[rgba(124,58,237,0.05)]">
                    ...
                    <span className="text-xs text-muted-foreground ml-auto">{group.tasks.length} tasks</span>
                  </div>
                  {group.tasks.map(task => {
                    ...
                    return (
                      <div key={task.id} className="flex items-center ml-8 border-t border-[rgba(124,58,237,0.03)] hover:bg-[rgba(124,58,237,0.05)]">
                        <div className="w-48 flex-shrink-0 px-3 py-2">
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            ...
                            {isOverdue && <span className="text-[10px] text-[#7C3AED]">Overdue</span>}
                          </div>
                        </div>
                        <div className="flex-1 relative h-8">
                          {start && (
                            ...
                            >
                              ...
                              />
                              <span className="absolute inset-0 flex items-center px-1.5 text-[10px] text-foreground truncate">
                                {task.name}
                              </span>
                            </button>
                          )}
      ...
      <div className="p-4 border-t border-[rgba(124,58,237,0.1)] flex items-center gap-4 text-xs text-muted-foreground">
