...
export function ReportsPage() {
  ...
  const statCards = [
    ...
    { icon: AlertCircle, label: 'Overdue', value: overdueTasks.toString(), subtext: 'Need attention', color: 'text-[#7C3AED]', bgColor: 'bg-[rgba(124,58,237,0.1)]' },
    ...
    <div className="p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-foreground mt-1">Performance metrics and insights</p>
        </div>
      ...
      {activeTab === 'analytics' && (
        <div className="space-y-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="glass-card rounded-xl p-6">
              ...
              <div className="space-y-3">
                {[
                  { status: 'approved', label: 'Approved', color: 'bg-[#A78BFA]' },
                  { status: 'completed', label: 'Completed', color: 'bg-[#8b5cf6]' },
                  { status: 'in_progress', label: 'In Progress', color: 'bg-[#7C3AED]' },
                  { status: 'blocked', label: 'Blocked', color: 'bg-[#7C3AED]' },
                  ...
                  return (
                    <div key={item.status}>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-sm text-foreground">{count} ({pct}%)</span>
                      </div>
                  ...
                  return (
                    <div key={emp.id} className="flex items-center gap-3 p-3 bg-[#FAF5FF] border border-[#E9D5FF] rounded-lg">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-xs text-foreground">{assignedTasks.length} tasks · {completed} completed</p>
                      </div>
                      <div className="w-16 h-1.5 bg-[#E9D5FF] rounded-full overflow-hidden">
                        <div
                          className="h-full bg-gradient-to-r from-[#7C3AED] to-[#7C3AED] rounded-full"
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {visibleGoals.map(goal => {
                  ...
                  return (
                    <div key={goal.id}>
                      ...
                      <p className="text-xs text-muted-foreground mt-1">{completed} of {goalTasks.length} tasks</p>
                    </div>
                  );
                })}
              </div>
            ...
            <div className="glass-card rounded-xl p-6">
              <h3 className="text-lg font-semibold text-foreground mb-4">Defect Summary</h3>
              <div className="space-y-3">
                <div className="flex items-center justify-between p-3 bg-[#FAF5FF] border border-[#E9D5FF] rounded-lg">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm text-foreground">Open Defects</span>
                  </div>
                  <span className="text-lg font-bold text-[#7C3AED]">{visibleDefects.filter(d => !['resolved', 'closed'].includes(d.status)).length}</span>
                </div>
                <div className="flex items-center justify-between p-3 bg-[#FAF5FF] border border-[#E9D5FF] rounded-lg">
                  <div className="flex items-center gap-3">
                  ...
                  <span className="text-lg font-bold text-[#A78BFA]">{visibleDefects.filter(d => d.status === 'resolved').length}</span>
                </div>
                <div className="flex items-center justify-between p-3 bg-[#FAF5FF] border border-[#E9D5FF] rounded-lg">
                  <div className="flex items-center gap-3">
        ...
        <div className="space-y-4">
          ...
          {reports.length === 0 ? (
            <div className="glass-card rounded-xl p-12 text-center">
              ...
              <p className="text-foreground">No reports generated yet.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {reports.map(report => (
                <div key={report.id} className="glass-card rounded-xl p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm text-foreground">Generated {report.createdAt ? report.createdAt.toLocaleDateString() : 'N/A'}</p>
                    </div>
