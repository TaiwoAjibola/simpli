  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-3xl font-bold text-[#f0f0f5] mb-2">Insights</h1>
          <p className="text-[#6b6b80]">Performance, activity, and history in one place</p>
        </div>
        <select
          value={selectedAppId}
      ...
      <div className="flex items-center bg-[#12121a] border border-[rgba(0,229,255,0.1)] w-fit mb-6">
        {tabs.map(t => {
          const Icon = t.icon;
          return (
            <button
...
function AnalyticsTab({ apps, selectedAppId, appGoals, appTasks, appSubtasks, employees }: {
  ...
  const statCards = [
    ...
    { icon: Clock, label: 'In Progress', value: inProgressTasks.toString(), subtext: `${blockedTasks} blocked`, color: 'text-[#00e5ff]', bgColor: 'bg-[rgba(0,229,255,0.1)]' },
  ...
  if (apps.length === 0) {
    return (
      <div className="text-center py-12 bg-[#12121a] border border-[rgba(0,229,255,0.1)]">
        ...
        <p className="text-[#6b6b80]">No apps yet — create an app to see analytics</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {statCards.map(card => {
          ...
          return (
            <div key={card.label} className="bg-[#12121a] border border-[rgba(0,229,255,0.1)] p-6">
              <div className={`p-2 mb-3 w-fit ${card.bgColor}`}>
                <Icon className={`w-5 h-5 ${card.color}`} />
              </div>
              <p className="text-2xl font-bold text-[#f0f0f5]">{card.value}</p>
              <p className="text-sm text-[#6b6b80] mt-1">{card.label}</p>
              <p className="text-xs text-[#6b6b80] mt-1">{card.subtext}</p>
            </div>
          );
        })}
      </div>
        ...
        <div className="bg-[#12121a] border border-[rgba(0,229,255,0.1)] p-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            {Object.entries(priorityDistribution).map(([priority, count]) => (
              <div key={priority} className={`p-4 ${priorityBgMap[priority]} border border-[rgba(0,229,255,0.1)]`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-[#12121a] border border-[rgba(0,229,255,0.1)] p-6">
          ...
          <div className="space-y-4">
            {goalProgress.length === 0 && <p className="text-sm text-[#6b6b80]">No goals yet</p>}
        ...
        <div className="bg-[#12121a] border border-[rgba(0,229,255,0.1)] p-6">
          ...
          <div className="space-y-3">
            {employeeWorkload.length === 0 && <p className="text-sm text-[#6b6b80]">No assignments yet</p>}
      ...
      <div className="bg-[#12121a] border border-[rgba(0,229,255,0.1)] p-6">
        ...
        <div className="space-y-3">
          {upcomingDeadlines.length === 0 && <p className="text-sm text-[#6b6b80]">No upcoming deadlines</p>}
            ...
            return (
              <div key={task.id} className="flex items-center justify-between p-3 bg-[#1a1a2e] border border-[rgba(0,229,255,0.1)]">
                <div className="flex items-center gap-3">
                  {isOverdue ? <XCircle className="w-5 h-5 text-[#ff3b5c]" /> : isDueToday ? <AlertCircle className="w-5 h-5 text-[#f59e0b]" /> : <Calendar className="w-5 h-5 text-[#00e5ff]" />}
                  <div>
        ...
        <div className="flex items-center bg-[#1a1a2e] border border-[rgba(0,229,255,0.1)]">
          {(['all', 'goal', 'task'] as const).map(t => (
            <button
              ...
              className={`px-4 py-2 text-sm capitalize ${filterType === t ? 'text-[#00e5ff] bg-[rgba(0,229,255,0.1)]' : 'text-[#6b6b80]'}`}
            >
              {t}
            </button>
          ))}
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        ...
        <div className="bg-[#12121a] border border-[rgba(0,229,255,0.1)] p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-[#f0f0f5]">{completedSubtaskCount}</p>
            ...
            <div key={item.id} className="border-b border-[rgba(0,229,255,0.05)] last:border-b-0">
              <button onClick={() => toggleExpand(item.id)} className="w-full flex items-center gap-4 p-4 hover:bg-[rgba(255,255,255,0.02)] text-left">
                ...
                <div className={`p-2 ${item.type === 'goal' ? 'bg-[rgba(139,92,246,0.1)]' : 'bg-[rgba(0,229,255,0.1)]'}`}>
                  {item.type === 'goal' ? <Target className="w-4 h-4 text-[#8b5cf6]" /> : <CheckCircle className="w-4 h-4 text-[#00e5ff]" />}
                </div>
                <div className="flex-1 min-w-0">
                ...
                <div className="px-4 pb-4 pl-14">
                  ...
                  {item.completedAt && (
                    <div className="mt-4 pt-3 border-t border-[rgba(0,229,255,0.1)]">
                      <p className="text-xs text-[#6b6b80]">Completed on {formatDate(item.completedAt)}</p>
                    </div>
                  )}
                </div>
              )}
