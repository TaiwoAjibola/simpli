    ...
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Rules that react to work events with permission-protected, idempotent actions.</p>
        </div>
        {canManage && (
          <button
      ...
      {showForm && (
        <div className="bg-white border border-[rgba(124,58,237,0.1)] p-5 rounded-lg mb-6 space-y-3">
          <div className="grid grid-cols-2 gap-4">
            <input
              value={form.name}
              onChange={e => setForm({ ...form, name: e.target.value })}
              placeholder="Rule name (e.g. Move to QA on PR open)"
              className="px-3 py-2 bg-[#020617] border border-[rgba(124,58,237,0.2)] text-foreground text-sm rounded"
            />
            <select
              value={form.event}
      ...
      <div className="space-y-3">
        ...
        {automations.map(a => (
          <div key={a.id} className="bg-white border border-[rgba(124,58,237,0.1)] p-4 rounded-lg">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="font-medium text-foreground">{a.name}</h3>
                <p className="text-xs text-muted-foreground mt-0.5">
                  When <span className="text-[#7C3AED]">{a.trigger.event.replace(/_/g, ' ')}</span>
                  ...
                  {' → '}set status to <span className="text-[#A78BFA]">{a.action.setStatus || '-'}</span>
                </p>
              </div>
              <div className="flex items-center gap-2">
                {canManage && (
                  <>
                    ...
                    >
                      {a.enabled ? <ToggleRight className="w-5 h-5 text-[#A78BFA]" /> : <ToggleLeft className="w-5 h-5" />}
                    </button>
                    <button onClick={() => setExpanded(expanded === a.id ? null : a.id)} className="p-1.5 text-muted-foreground hover:text-[#7C3AED]">
                      <History className="w-4 h-4" />
                    </button>
                    <button onClick={() => deleteAutomation(a.id)} className="p-1.5 text-muted-foreground hover:text-[#ef4444]">
            ...
            {expanded === a.id && (
              <div className="mt-3 border-t border-[rgba(124,58,237,0.1)] pt-3">
                <p className="text-xs text-muted-foreground mb-2">Run history ({a.runHistory?.length || 0})</p>
                <div className="max-h-40 overflow-y-auto space-y-1">
                  {(a.runHistory || []).slice().reverse().map(r => (
                    <div key={r.runId} className="flex items-center gap-2 text-xs">
                      <span className={`px-1.5 py-0.5 rounded ${r.outcome === 'applied' ? 'bg-[rgba(124,58,237,0.15)] text-[#A78BFA]' : 'bg-[rgba(239,68,68,0.15)] text-[#ef4444]'}`}>
                        {r.outcome}
                      </span>
                      <span className="text-foreground">{r.workKind} {r.workId}</span>
                      <span className="text-muted-foreground">{r.runAt.toLocaleString()}</span>
