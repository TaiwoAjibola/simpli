    ...
    <div className="p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-[#f0f0f5] flex items-center gap-2">
            <LayoutTemplate className="w-6 h-6 text-[#00e5ff]" />
            Work Templates
          </h1>
          <p className="text-sm text-[#6b6b80] mt-1">Reusable task, action point, and defect templates.</p>
        </div>
        {canManage && (
          <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {workTemplates.length === 0 && (
          <p className="text-sm text-[#6b6b80]">No templates yet. Create one to get started.</p>
        )}
        {workTemplates.map(t => (
          <div key={t.id} className="bg-[#161b22] border border-[rgba(0,229,255,0.1)] p-4 rounded-lg">
            <div className="flex items-start justify-between">
              <div>
                <h3 className="font-medium text-[#f0f0f5]">{t.name}</h3>
                <p className="text-xs text-[#6b6b80] capitalize">{t.workKind.replace('_', ' ')}</p>
              </div>
              <div className="flex gap-1">
                ...
                <button
                  ...
                  className="p-1.5 text-[#6b6b80] hover:bg-[rgba(0,229,255,0.1)] rounded"
                  title="Copy title"
                >
                ...
                {canManage && (
                  <button
                    ...
                    className="p-1.5 text-[#6b6b80] hover:text-[#ef4444] rounded"
                    title="Delete"
                  >
            ...
            {t.description && <p className="text-sm text-[#6b6b80] mt-2">{t.description}</p>}
