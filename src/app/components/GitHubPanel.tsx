  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        {repoOptions.length === 0 && <span className="text-xs text-[#6b6b80]">No repositories configured</span>}
      </div>
          ...
          {STATUS_ORDER.map((s, i) => {
            ...
            return (
              <React.Fragment key={s}>
                {i > 0 && <div className={`h-0.5 w-4 shrink-0 ${reached ? 'bg-[#00e5ff]' : 'bg-[#2a2a3e]'}`} />}
                <span
      ...
      {(currentRepo || github?.repositoryId) && github?.branchName && (
        <div className="bg-[#161b22] border border-[rgba(0,229,255,0.1)] p-3 rounded space-y-2">
          <div className="flex items-center justify-between text-sm">
            <span className="flex items-center gap-2 text-[#f0f0f5]">
              <GitBranch className="w-4 h-4 text-[#00e5ff]" />
              Branch: {github.branchName}
              {github.branchUrl && (
                <a href={github.branchUrl} target="_blank" rel="noreferrer" className="text-[#6b6b80] hover:text-[#00e5ff]">
          ...
          {github.commits && github.commits.length > 0 && (
            <div className="space-y-1">
              {github.commits.slice(0, 5).map(c => (
                <div key={c.sha} className="flex items-center gap-2 text-xs text-[#6b6b80]">
          ...
          {github.pullRequest && (
            <div className="flex items-center justify-between border-t border-[rgba(0,229,255,0.1)] pt-2 text-sm">
              <span className="flex items-center gap-2 text-[#f0f0f5]">
              ...
              <div className="flex items-center gap-2 text-xs">
                ...
                {github.pullRequest.url && (
                  <a href={github.pullRequest.url} target="_blank" rel="noreferrer" className="text-[#00e5ff] hover:opacity-80">
                    <ExternalLink className="w-3.5 h-3.5" />
                  </a>
                )}
              </div>
          ...
          {!github?.branchName && (
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 bg-[#0d1117] border border-[rgba(0,229,255,0.2)] text-[#f0f0f5] text-sm px-2 py-1.5 rounded"
              />
              <button
                onClick={handleCreateBranch}
                disabled={busy === 'branch' || !branchName.trim()}
                className="flex items-center gap-1 px-3 py-1.5 bg-[#00e5ff] text-[#0a0a0f] text-sm font-medium hover:bg-[#00c4e0] rounded disabled:opacity-50"
              >
      ...
      {!canDev && (
        <p className="text-xs text-[#6b6b80]">You need develop_work or manage_repositories permission to create branches/PRs.</p>
      )}
      ...
      <div className="flex items-center gap-1 text-xs text-[#6b6b80]">
        ...
        {github?.status === 'closed' && <XCircle className="w-3 h-3 text-[#6b6b80]" />}
