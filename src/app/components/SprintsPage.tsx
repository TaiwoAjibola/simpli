  ...
  return (
    <div className="p-4 lg:p-8">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl lg:text-3xl font-bold text-[#F8FAFC] mb-1">Sprints</h1>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {canManage && (
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 bg-[#22C55E] text-[#020617] font-medium text-sm hover:bg-[#00d5ef] transition"
            >
      ...
      {showForm && (
        <div className="bg-[#0F172A] border border-[rgba(34,197,94,0.2)] p-6 mb-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-[#F8FAFC]">
              {editingSprint ? 'Edit Sprint' : 'New Sprint'}
            </h2>
            <button onClick={resetForm} className="text-[#94A3B8] hover:text-[#F8FAFC]">✕</button>
          </div>
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
            <div>
              <label className="block text-xs text-[#94A3B8] mb-1">App</label>
              <select
            ...
            <div>
              <label className="block text-xs text-[#94A3B8] mb-1">Goal / Theme (optional)</label>
              <input
                type="text"
                value={formData.goal}
          ...
          <div className="flex gap-2 mt-5">
            <button
              ...
              className="px-4 py-2 bg-[#22C55E] text-[#020617] font-medium text-sm hover:bg-[#00d5ef] transition"
            >
              {editingSprint ? 'Save Changes' : 'Create Sprint'}
            </button>
            <button
              onClick={resetForm}
              className="px-4 py-2 text-[#94A3B8] hover:text-[#F8FAFC] text-sm border border-[rgba(255,255,255,0.1)]"
            >
              Cancel
            </button>
          </div>
      ...
      <div className="space-y-4">
        {filteredSprints.length === 0 && (
          <div className="bg-[#0F172A] border border-[rgba(34,197,94,0.1)] p-12 text-center">
            ...
            <p className="text-[#94A3B8]">No sprints yet. Create your first sprint to start planning work.</p>
          </div>
        )}
        ...
        {filteredSprints.map(sprint => {
          ...
          return (
            <div key={sprint.id} className="bg-[#0F172A] border border-[rgba(34,197,94,0.1)]">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <button onClick={() => toggleExpand(sprint.id)} className="flex items-center gap-2 text-left group">
                      ...
                      <h2 className="text-lg font-semibold text-[#F8FAFC] group-hover:text-[#22C55E] transition">
                        {sprint.name}
                      </h2>
                    </button>
                  ...
                  <div className="flex items-center gap-4 mt-2 text-xs text-[#94A3B8] flex-wrap">
                    {app && (
                ...
                {canManage && (
                  <div className="flex items-center gap-1 flex-shrink-0">
                    ...
                    <button
                      ...
                      className="p-2 text-[#22C55E] hover:bg-[rgba(34,197,94,0.1)] rounded transition"
                      title="Edit sprint"
                    >
              ...
              {isExpanded && (
                <div className="border-t border-[rgba(34,197,94,0.1)] p-5">
                  {items.length === 0 ? (
                    <div>
                      <p className="text-xs text-[#94A3B8] mb-3">No work assigned yet. Pick from unassigned work below.</p>
                      <UnassignedList
...
function UnassignedList({ items, sprintId, onAssign, emptyText }: {
  ...
  if (items.length === 0) {
    return <p className="text-xs text-[#94A3B8]">{emptyText}</p>;
  }
  return (
    <div>
      <p className="text-xs text-[#94A3B8] uppercase tracking-wider mb-2">Unassigned work for this app</p>
      <div className="border border-[rgba(34,197,94,0.1)] divide-y divide-[rgba(34,197,94,0.05)]">
        {items.slice(0, 15).map(item => (
          <div key={`${item.workKind}-${item.id}`} className="flex items-center gap-3 px-3 py-2">
            <span className={`text-xs font-medium px-2 py-0.5 ${item.workKind === 'defect' ? 'text-[#dc2626] bg-[rgba(220,38,38,0.1)]' : item.workKind === 'action_point' ? 'text-[#f59e0b] bg-[rgba(245,158,11,0.1)]' : 'text-[#22C55E] bg-[rgba(34,197,94,0.1)]'}`}>
              {item.workKind === 'defect' ? 'DEFECT' : item.workKind === 'action_point' ? 'AP' : 'TASK'}
            </span>
            <span className="flex-1 min-w-0 truncate text-sm text-[#F8FAFC]">{item.title}</span>
            {item.code && <span className="text-xs font-mono text-[#94A3B8]">{item.code}</span>}
            <button
              onClick={() => onAssign(item, sprintId)}
              className="text-xs px-2 py-1 text-[#22C55E] border border-[rgba(34,197,94,0.2)] hover:bg-[rgba(34,197,94,0.1)] transition flex-shrink-0"
            >
              Add
            </button>
          </div>
      ...
      {items.length > 15 && (
        <p className="text-xs text-[#94A3B8] mt-2">+{items.length - 15} more unassigned items</p>
      )}
    </div>
  );
}
...
function WorkRow({ item, onUnassign }: { item: WorkItem; onUnassign: () => void }) {
  ...
  return (
    <div className="flex items-center gap-3 px-3 py-2 bg-[#0F172A] border border-[rgba(34,197,94,0.05)]">
      {kindIcon}
      <span className="flex-1 min-w-0 truncate text-sm text-[#F8FAFC]">{item.title}</span>
      {item.code && <span className="text-xs font-mono text-[#94A3B8]">{item.code}</span>}
      <span className="text-xs capitalize text-[#94A3B8]">{item.status.replace(/_/g, ' ')}</span>
      <button
        onClick={onUnassign}
        className="text-xs px-2 py-1 text-[#94A3B8] hover:text-[#ff3b5c] border border-[rgba(255,255,255,0.1)] transition flex-shrink-0"
